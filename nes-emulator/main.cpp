...
namespace sn
{
    void parseControllerConf(string filepath,
                            vector<sf::Keyboard::Key>& p1,
                            vector<sf::Keyboard::Key>& p2);
}

int main(int argc, char** argv)
{
    ofstream logFile ("simplenes.log"), cpuTraceFile;
    sn::TeeStream logTee (logFile, cout);

    if (logFile.is_open() && logFile.good())
        sn::Log::get().setLogStream(logTee);
    else
        sn::Log::get().setLogStream(cout);
    ...
    // Default keybindings
    vector<sf::Keyboard::Key> p1 {sf::Keyboard::J, sf::Keyboard::K, sf::Keyboard::RShift, sf::Keyboard::Return,
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (arg == "-h" || arg == "--help")
        {
            cout << "SimpleNES is a simple NES emulator.\n"
                 ...
                 << "                       Scale of 1 corresponds to " << sn::NESVideoWidth << "x" << sn::NESVideoHeight << endl
        ...
        else if (strcmp(argv[i], "-s") == 0 || strcmp(argv[i], "--scale") == 0)
        {
            ...
            if (i + 1 < argc && ss << argv[i + 1] && ss >> scale)
                ...
                LOG(sn::Error) << "Setting scale from argument failed" << endl;
            ++i;
        }
        else if (strcmp(argv[i], "-w") == 0 || strcmp(argv[i], "--width") == 0)
        {
            ...
            if (i + 1 < argc && ss << argv[i + 1] && ss >> width)
                ...
                LOG(sn::Error) << "Setting width from argument failed" << endl;
            ++i;
        }
        else if (strcmp(argv[i], "-H") == 0 || strcmp(argv[i], "--height") == 0)
        {
            ...
            if (i + 1 < argc && ss << argv[i + 1] && ss >> height)
                ...
                LOG(sn::Error) << "Setting height from argument failed" << endl;
            ++i;
        }
        else if (strcmp(argv[i], "-C") == 0 || strcmp(argv[i], "--conf") == 0)
        {
            if (i + 1 < argc)
                ...
                LOG(sn::Error) << "Setting keybindings.conf's path from argument failed" << endl;
            ++i;
        }
        else if (argv[i][0] != '-')
            path = argv[i];
    ...
    if (path.empty())
    {
        cout << "Argument required: ROM path" << endl;
        return 1;
    }

    sn::parseControllerConf(move(keybindingsPath), p1, p2);
